...
def get_args():
    ...
    # ======== NEW: 通过 --resume 指定要恢复的checkpoint(如果有) ========
    ...
    # ======== NEW: early stop 的耐心次数, 默认3 =========
    parser.add_argument("--early_stop_patience", type=int, default=3, help="stop if val acc not improve after N epochs")
...
def main(args, ds_init):
    ...
    print("Update frequent =", args.update_freq)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if "epoch" in checkpoint:
                ...
                print(f"Setting start_epoch to {args.start_epoch} based on resume checkpoint.")
            # load model weights
            ...
            print(f"=> Resume success, now start epoch {args.start_epoch}")
        else:
            print(f"Warning: no checkpoint file found at {args.resume}, ignoring resume...")
        ...
        if args.output_dir and args.save_ckpt:
            if ((epoch + 1) % args.save_ckpt_freq == 0) or ((epoch + 1) == args.epochs):
                utils.save_model(
                    ...
                    epoch=epoch,      # <--- 关键: 让checkpoint["epoch"] = epoch
                    model_ema=None,
                )
        ...
        if (dataset_val is not None) and (not args.disable_eval_during_finetuning):
            ...
            if max_accuracy < test_stats["acc1"]:
                ...
                if no_improve_count >= args.early_stop_patience:
                    print(f"Early stopping triggered (no improvement >= {args.early_stop_patience} epochs).")
                    break
    ...
    # ============ Training done, load best for test ============
