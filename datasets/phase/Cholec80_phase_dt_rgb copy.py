...
def convert_masks_depth(mask_path, depth_path, frame_id):
    ...
    return final_11ch
    # return final_11ch, mask_file_path, depth_file_path  # === 返回mask_file_path, depth_file_path用于debug
    ...
    def _video_batch_loader(self, duration, cur_frame_id, video_id, index):
        ...
        for fid in sampled_list:
            ...
            try:
                ...
                # md_data, debug_mask_path, debug_depth_path = convert_masks_depth(mask_dir, depth_dir, real_frame_id)
                # # === Debug Print ===
                # print("=== Debug Info (online) ===")
                # print(f"RGB Path   : {rgb_path}")
                # print(f"Mask Path  : {debug_mask_path}")
                # print(f"Depth Path : {debug_depth_path}")
                # print("---------------------------")
